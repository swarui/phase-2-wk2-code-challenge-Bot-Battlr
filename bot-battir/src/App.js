...
const App = () => {
  // Define state variables using state hooks
  ...
  useEffect(() => {
    ...
    // Check and add robots to MyBotArmy
    if (storedBotArmy) {
  ...
  const enlistBot = (botId) => {
    // Find robot by unique id in bots box.    const selectedBot = bots.find((bot) => bot.id === botId);
  ...
  const addBot = (newBot) => {
    // Generate a unique ID for the new bot
    newBot.id = Date.now();
  ...
  const dischargeBot = (botId) => {
    // Filter out the clicked robot from the team using its unique 'id'
